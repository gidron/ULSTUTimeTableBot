"""Аккаунты владельца/оператора бота, недоступные для бана, удаления и снятия админки делегированными админами."""
...
def can_admin_revoke_admin_from_target(*, actor_tg_id: int | str, target_tg_id: int | str) -> bool:
    """Делегированный админ не может снять админ-права с защищённого оператора (см. developer_chat_id)."""
    if int(actor_tg_id) == int(target_tg_id):
        return True
    return not is_protected_operator(target_tg_id)
